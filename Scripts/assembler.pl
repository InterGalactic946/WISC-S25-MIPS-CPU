...
if(@ARGV < 1) { print "Usage: assembler.pl <input assembly file> > outputFile\n"; exit; }
...
my %numArgs = ( qw/ADD 3 SUB 3 PADDSB 3 XOR 3 SLL 3 SRA 3 ROR 3 RED 3 LW 3 SW 3 LLB 2 LHB 2 B 2 BR 2 PCS 1 HLT 0/);



my %opcode = ( qw/ADD 0000 SUB 0001 XOR 0010 RED 0011 SLL 0100 SRA 0101 ROR 0110 PADDSB 0111 LW 1000 SW 1001 LLB 1010 LHB 1011 B 1100 BR 1101 PCS 1110 HLT 1111/);
...
while(<IN>) {
  ...
  if( /^\s*(\S+)\s*(.*)/ ) {
	...
	$bits .= "0000";
	  ...
	  if($args[1] !~ /[a-zA-Z]/) { print STDERR "Error: Invalid label name: \"$args[1]\""; exit; }
...
#      elsif($instr =~ /^(JAL)$/) {
#
#	  if($args[0] !~ /[a-zA-Z]/) { print STDERR "Error: Invalid label name: \"$args[0]\""; exit; }
#
#	  $bits .= "|" . $args[0] . "|12|J|";
#
#      }
